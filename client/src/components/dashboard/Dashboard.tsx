...
const categoryItemStyle = (first: boolean, second: boolean, third: boolean) =>
	cn('group aspect-h-1 aspect-w-2 overflow-hidden rounded-lg', {
		'sm:aspect-h-1 sm:aspect-w-1 sm:row-span-2': first,
		'sm:aspect-none sm:relative sm:h-full': second || third
	});

const categoryImgStyle = (first: boolean) =>
	cn('object-cover object-center group-hover:opacity-75', {
		'sm:absolute sm:inset-0 sm:h-full sm:w-full': !first
	});
...
const Dashboard = () => {
	...
	const fetchCategories = async () => {
		try {
			const response = await fetch('https://64a99c4c8b9afaf4844ae636.mockapi.io/categories');
			if (!response.ok) {
	...
	return (
		<div className='bg-white'>
			{/* Hero section */}
			<div className='pb-80 pt-16 sm:pb-40 sm:pt-24 lg:pb-48 lg:pt-40 text-left'>
			...
			<main>
				{/* Category section */}
				<section aria-labelledby='category-heading' className='mt-20'>
					<div className='mx-auto max-w-7xl px-4 py-24 sm:px-6 sm:py-32 lg:px-8'>
						...
						<div className='mt-6 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:grid-rows-2 sm:gap-x-6 lg:gap-8'>
							{categories?.map((category: any, idx) => (
								<div key={category.id} className={categoryItemStyle(idx === 0, idx === 1, idx === 2)}>
									<img src={category.image} alt={category.name} className={categoryImgStyle(idx === 0)} />
									<div
										aria-hidden='true'
